...
@section('content')
    <section>
        ...
        <form method="POST" action="{{ url('ad/'.$ad->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <!-- Post Your ad start -->
            <fieldset class="border border-gary p-4 mb-5">
                <div class="row">
                    ...
                    <div class="col-lg-6">
                        ...
                        <input name="title" value="{{$ad->title}}" type="text" class="border w-100 p-2 bg-white text-capitalize"
                            placeholder="Ad title go There">
                        <h6 class="font-weight-bold pt-4 pb-1">Health:</h6>
                        <input type="range" aria-valuenow="{{$ad->health}}" name="health" step="10" min="0" max="100"
                            class="border w-100 p-2 bg-white text-capitalize">

                        <h6 class="font-weight-bold pt-4 pb-1">Brand:</h6>
                        <input name="brand" value="{{$ad->brand}}" type="text" class="border w-100 p-2 bg-white text-capitalize"
                        ...
                        <div class="row px-3">
                            <div class="col-lg-4 mr-lg-4 my-2 rounded bg-white">
                                <input type="radio" @if ($ad->condition == 0)
                                    checked
                                @endif name="condition" value="1" id="personal">
                                <label for="personal" class="py-2">Fresh</label>
                            </div>
                            <div class="col-lg-4 mr-lg-4 my-2 rounded bg-white ">
                                <input type="radio" @if ($ad->condition == 1)
                                checked
                            @endif name="condition" value="0" id="business">
                                <label for="business" class="py-2">Used</label>
                            </div>
                        </div>
                        <h6 class="font-weight-bold pt-4 pb-1">Description:</h6>
                        <textarea name="description" class="border p-3 w-100" rows="7"
                            placeholder="Write details about your product">{{$ad->description}}</textarea>
                    </div>
                    <div class="col-lg-6">
                        ...
                        <select name="category_id" id="inputGroupSelect" class="w-100"
                            ...
                            @foreach (\App\Models\Category::all() as $category)
                                <option @if($category->id == $ad->category_id)
                                     selected
                        ...
                        <div class="price">
                            ...
                            <input name="model" type="text" value="{{$ad->model}}" class="border w-100 p-2 bg-white text-capitalize"
                                placeholder="Build Model">
                            <h6 class="font-weight-bold pt-4 pb-1">Specifications:</h6>
                            <input name="specifications" value="{{$ad->specifications}}" type="text"
                            ...
                            <input name="location" value="{{$ad->location}}" type="text" class="border w-100 p-2 bg-white text-capitalize"
                                placeholder="Ad Location">

                            <h6 class="font-weight-bold pt-4 pb-1">Item Price (Rs. PKR):</h6>
                            <div class="row px-3">
                                <div class="col-lg-4 mr-lg-4 rounded bg-white my-2 ">
                                    <input type="text" value="{{$ad->price}}" name="price" class="border-0 py-2 w-100 price"
                                        placeholder="Price" id="price">
                                </div>
                                <div class="col-lg-4 mrx-4 rounded bg-white my-2 ">
                                    <input type="checkbox" @if ($ad->is_negotiable == 1)
                                        checked
                                    @endif name="is_negotiable" value="1" id="Negotiable">
            ...
            <button type="submit" class="btn btn-primary d-block mt-2">Post Your Ad</button>
                ...
                @else
                <form method="POST" action="{{ url('job/'.$job->id) }}" enctype="multipart/form-data">
                    @method('PUT')
                            @csrf
                            <fieldset class="border border-gary p-4 mb-5">
                                <div class="row">
